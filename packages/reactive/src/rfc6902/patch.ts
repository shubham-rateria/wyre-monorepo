...
export function add(
  ...
  if (endpoint.parent instanceof ObservableArray) {
    const path = `/${endpoint.key}`;
    operation.path = path;
    // @ts-ignore
    endpoint.parent.applyPatch(operation);
    return null;
  }
...
): MissingError | null | string | number {
...
export function replace(
  ...
  // this existence check treats arrays and observable arrays as a special case
  if (endpoint.parent instanceof ObservableArray) {
    const path = `/${endpoint.key}`;
    operation.path = path;
    // @ts-ignore
    endpoint.parent.applyPatch(operation);
    return null;
  } else if (Array.isArray(endpoint.parent)) {
...
export function apply(
  object: any,
  operation: Operation
): MissingError | InvalidOperationError | TestError | null | string | number {
