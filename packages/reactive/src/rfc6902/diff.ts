...
export function intersection(
  objects: ArrayLike<{ [index: string]: any }>
): string[] {
    ...
    if (memoized === undefined) {
      ...
      if (
        ...
        if (i > 0) {
          ...
          alternatives.push(
            appendArrayOperation(remove_base, remove_operation)
          );
        }
        if (j > 0) {
          // NOT leftmost column
        ...
        if (i > 0 && j > 0) {
          ...
          alternatives.push(
            appendArrayOperation(replace_base, replace_operation)
          );
        }
