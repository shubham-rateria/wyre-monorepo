...
const config = {
  ...
  KEYGEN_PRODUCT_TOKEN:
    "prod-bc4dfa97f8927592f15275b5bc8427580cf0f251d7168540a0345f49a2203ae3v3",
...
async function createRelease() {
  const res = await fetch(
    `https://api.keygen.sh/v1/accounts/${KEYGEN_ACCOUNT_ID}/releases`,
    {
      method: "POST",
      headers: {
  ...
  if (errors) {
    throw new Error(
      `failed to create release for product ${KEYGEN_PRODUCT_ID}: ${JSON.stringify(
        { errors }
      )}`
    );
  }
...
async function publishRelease(release) {
  const res = await fetch(
    `https://api.keygen.sh/v1/accounts/${KEYGEN_ACCOUNT_ID}/releases/${release.id}/actions/publish`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${KEYGEN_PRODUCT_TOKEN}`,
  ...
  if (errors) {
    throw new Error(
      `failed to publish release ${release.id}: ${JSON.stringify({ errors })}`
    );
  }
...
async function uploadArtifactForRelease({
  releaseId,
  type,
  file,
  checksum,
  ...
  const res = await fetch(
    `https://api.keygen.sh/v1/accounts/${KEYGEN_ACCOUNT_ID}/artifacts`,
    {
      redirect: "manual",
      method: "POST",
      headers: {
  ...
  if (errors) {
    throw new Error(
      `failed to create artifact for release ${releaseId}: ${JSON.stringify({
        errors,
      })}`
    );
  }
...
async function getManifestForPackage() {
  const res = await fetch(
    `https://api.keygen.sh/v1/accounts/${KEYGEN_ACCOUNT_ID}/artifacts/${PACKAGE_NAME}`,
    {
      redirect: "manual",
      method: "GET",
      headers: {
  ...
  if (errors) {
    throw new Error(
      `failed to retrieve manifest for ${PACKAGE_NAME}: ${JSON.stringify({
        errors,
      })}`
    );
  }
